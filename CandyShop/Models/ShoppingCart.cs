...
namespace CandyShop.Models
{
    public class ShoppingCart
    {
        ...
        {
            /*** Ephemeral Data ****/
            //Much of the data stored on computers,
            //such as the data stored in random access memory (RAM) and caches,
            //is temporary and therefore referred to as "ephemeral" (which means transitory, or existing only briefly).
            //These temporary, transient files are deleted as often as every few hours.

            
            // Gets the browser? Session, if none found, returns null.
            ISession session = services.GetRequiredService<IHttpContextAccessor>()?.HttpContext.Session;
            var context = services.GetService<AppDbContext>();

            // Gets CartId from ISession session(Browser Cache) if ISession session returned null,
            // A new Guid is created. (Global Unique Identifier)
            string cartId = session.GetString("CartId") ?? Guid.NewGuid().ToString();
            // Sets the session Id (Key: CartId, Value: cartId)
